...
// 搜索能组合出该魔法的最小槽位回路组合（从 1 槽递增，首个命中即为最小）
function findMinCombos(magic, selectedIds, comboSize, circuitMap) {
  for (let size = 1; size <= Math.min(comboSize, selectedIds.length); size++) {
    const validCombos = [];
    for (const combo of combinations(selectedIds, size)) {
...
export default function calcMagicListComp(selectedCrystalCircuit, comboSize, circuitMap) {
  ...
  const magicListComp = magicList.filter((magic) => {
    const validCombos = findMinCombos(magic, selectedCrystalCircuit, comboSize, circuitMap);
